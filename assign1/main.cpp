 ...
 * Modify the signatures of these functions so that they work as intended, and
...
void parse_csv(std::string filename, std::vector<Course> &courses) {
  ...
  while (ifs.peek() != EOF) {
    std::getline(ifs, current_line);
    auto info_list = split(current_line, ',');
    Course this_course{info_list[0], info_list[1], info_list[2]};
    courses.push_back(this_course);
  }
}
...
void write_courses_offered(std::vector<Course> &all_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::vector<Course> courses_to_be_deleted{};
  ...
  for (auto &course : all_courses) {
    if (course.quarter != "null") {
      ofs << course.title << ',' << course.number_of_units << ','
          << course.quarter << '\n';
      courses_to_be_deleted.push_back(course);
    }
  }
  for (auto &course : courses_to_be_deleted) {
